...
class TgBotApiImpl : public TgBotApi, tgbotxx::Bot {
   ...
   private:
     ...
     * otherwise.
     */
    bool answerCallbackQuery_impl(
        const std::string_view callbackQueryId,
        const std::string_view text = {}, bool showAlert = false) const override;
   ...
   public:
    ...
    void commandHandler(const std::string& command,
                        AuthContext::AccessLevel authflags, api::types::Message message);
    bool validateValidArgs(const CommandModule::Info * module, api::types::ParsedMessage* message);
