...
struct ParsedMessage : public Message {
    ...
    ParsedMessage(Message message, SplitMethod how) : Message(message) {
        ...
        const auto botCommandEnt =
            std::ranges::find_if(*entities, [](const auto& entity) {
                return entity.type == api::types::MessageEntity::Type::BotCommand &&
                       entity.offset == 0;
            });
    ...
    [[nodiscard]] typename internal::AttributeType<attr>::type get()
        const {
        if (!has<attr>()) {
            return {};
        }
    ...
    [[nodiscard]] typename internal::AttributeType<attr>::type get_or(
        typename internal::AttributeType<attr>::type default_value)
        const {
        if (!has<attr>()) {
    ...
    // Equality comparsion.
    template <internal::Attrs attr>
    bool is(MakeCRef<typename internal::AttributeType<attr>::type>
                object) const {
        if (!has({attr})) {
            return false;
        }
