 ...
 * For example, in a message with the command "/start extra_arg1 extra_arg2",
 * this function would return true for the message pointer that points to "/start".
 * 
...
bool hasExtArgs(const Message::Ptr &message);
  ...
  * Extended arguments are arguments passed to a command after a forward slash (/).
  * For example, in a message with the command "/start extra_arg1 extra_arg2",
  * this function would update extraargs with "extra_arg1 extra_arg2".
  * 
  * @param message the Telegram message to check
  * @param extraargs a reference to a string that will contain the extra arguments, if any
  */
