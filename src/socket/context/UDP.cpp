...
bool Context::UDP::writeNonBlocking(const uint8_t* data,
    ...
    auto bytes_wrote = socket_.async_send_to(boost::asio::buffer(data, length),
                                             ep, boost::asio::use_future);
...
bool Context::UDP::listen(listener_callback_t listener, bool block) {
    ...
    socket_.async_receive_from(
        ...
        [this, listener, sender_endpoint](boost::system::error_code ec,
                                          std::size_t /*bytes_received*/) {
            if (ec) {
                LOG(ERROR) << "UDP::listen: Error receiving first packet: "
                           << ec.message();
                return;
            }
            ...
            LOG(INFO) << "UDP::listen: Received first packet from "
                      << remoteAddress();
    ...
    // Note: Don't call io_context.run() here as it's already running in the
    // constructor's thread The block parameter is ignored for UDP as the
    // io_context is managed by the constructor
    if (block) {
        io_context.run();
    }
    return true;
...
bool Context::UDP::connect(const RemoteEndpoint& endpoint) {
    try {
        ...
        // For UDP, send a handshake packet to trigger the server's listen
        // callback
        uint8_t handshake = 0;
...
bool Context::UDP::abortConnections() {
    io_context.stop();
    return close();
}
