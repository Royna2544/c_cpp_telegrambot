...
mod tests {
    ...
    async fn test_git_clone_success() {
        ...
        executor.add_response(MockResponse {
        ...
        let result = executor.execute(
            "git",
            &["clone", "--depth=1", "https://github.com/torvalds/linux.git"],
            None,
            &[],
            tx,
            None,
    ...
    async fn test_repo_sync_sequence() {
        ...
        executor.add_success(
            "repo",
            vec!["Repo command succeeded.".to_string()]
        );
        ...
        executor.add_success(
            "bash",
            vec!["Build completed successfully".to_string()]
        );

        // Execute repo init
        let (tx1, _rx1) = mpsc::channel(100);
        let result1 = executor.execute("repo", &["init"], None, &[], tx1, None).await;
        assert!(result1.unwrap().success);

        // Execute repo sync
        let (tx2, _rx2) = mpsc::channel(100);
        let result2 = executor.execute("repo", &["sync"], None, &[], tx2, None).await;
        assert!(result2.unwrap().success);

        // Execute build script
        let (tx3, _rx3) = mpsc::channel(100);
        let result3 = executor.execute("bash", &["build.sh"], None, &[], tx3, None).await;
    ...
    async fn test_insufficient_responses() {
        ...
        let executor = MockCommandExecutor::new();
        executor.add_success("git", vec!["Success".to_string()]);

        // First call should succeed
        let (tx1, _rx1) = mpsc::channel(100);
        let result1 = executor.execute("git", &["clone"], None, &[], tx1, None).await;
        assert!(result1.is_ok());

        // Second call should fail (no response configured)
        let (tx2, _rx2) = mpsc::channel(100);
        let result2 = executor.execute("git", &["clone"], None, &[], tx2, None).await;
    ...
    async fn test_build_with_cancellation() {
        ...
        let exec_handle = tokio::spawn(async move {
            executor.execute("make", &[], None, &[], tx, Some(kill_rx)).await
        });
