...
pub trait LogErr<T, E> {
    /// Logs a warning if the Result is Err, then returns the Result unchanged.
    fn warn_err(self, msg: &str) -> Result<T, E>;
    /// Same as above, but takes a String message
    fn warn_err_string(self, msg: String) -> Result<T, E>;
    /// Logs an error with a message generated by the provided closure if the Result is Err, then returns the Result unchanged.
    fn log_if_err(self, msg: &str) -> Result<T, E>;
    /// Logs an error with a message generated by the provided closure if the Result is Err, then returns the Result unchanged.
    fn log_if_err_string(self, msg: String) -> Result<T, E>;

    /// Same as above, but for dynamic messages (lazy evaluation)
    fn warn_with(self, f: impl FnOnce() -> String) -> Result<T, E>;
}

// 2. Implement the Trait for Result<T, E>
impl<T, E: Display> LogErr<T, E> for Result<T, E> {
    fn warn_err(self, msg: &str) -> Result<T, E> {
        self.inspect_err(|e| warn!("{}: {}", msg, e))
    }
