 ...
 * supported() function can be used to determine if a particular RNG is
...
struct RNGBase {
    /**
     * @brief      Determines if the RNG is supported on the system.
     *
     * @return     `true` if the RNG is supported, `false` otherwise.
     */
...
struct StdCpp : RNGBase {
    ...
    bool supported(void) const override { return true; }
...
static std::shared_ptr<RNGBase> getRNG(void) {
    static const std::vector<std::shared_ptr<RNGBase>> rngs = {
#ifdef RDRAND_MAYBE_SUPPORTED
        std::make_shared<RDRand>(),
#endif
#ifdef KERNELRAND_MAYBE_SUPPORTED
        std::make_shared<KernelRand>(),
#endif
        std::make_shared<StdCpp>(),
    };
    static std::shared_ptr<RNGBase> rng;
