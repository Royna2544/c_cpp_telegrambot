...
std::optional<int> ProtoDatabaseBase::findByUid(const RepeatedField<UserId>* list, const UserId uid) const {
...
void ProtoDatabaseBase::_addToDatabase(const Message::Ptr& message, RepeatedField<UserId>* list) {
...
void ProtoDatabaseBase::_removeFromDatabase(const Message::Ptr& message, RepeatedField<UserId>* list) {
    if (message->replyToMessage && message->replyToMessage->from) {
        const User::Ptr user = message->replyToMessage->from;
        const UserId id = user->id;
        do {
...
void ProtoDatabase::onAlreadyExist(const Message::Ptr& message, const User::Ptr& who,
...
void ProtoDatabase::onNotFound(const Message::Ptr& message, const User::Ptr& who,
                               const ProtoDatabaseBase* which ) const {
    bot_sendReplyMessage(_bot, message, appendListName("not found in", who, which));
}
void ProtoDatabase::onRemoved(const Message::Ptr& message, const User::Ptr& who,
                              const ProtoDatabaseBase* which) const {
    bot_sendReplyMessage(_bot, message, appendListName("removed from", who, which));
}
...
std::string ProtoDatabase::appendListName(const std::string& op, const User::Ptr from,
