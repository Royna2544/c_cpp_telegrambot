...
using Accessor = SingleThreadCtrlTestAccessors<
...
static void updateTimeStamp(std::vector<Message::Ptr>& inBuf,
...
struct MockSpamBlockTest : public SpamBlockBase {
    ...
    MOCK_METHOD(void, handleUserAndMessagePair,
                (PerChatHandleConstRef, OneChatIterator, const size_t,
                 const char*));
    void checks(PerChatHandleConstRef e, OneChatIterator it, const size_t th,
                const char* n) {
...
TEST(SpamBlockTest, GeneralSpamEntryOverThreshold) {
    ...
    // TODO: Check why the function is called twice; can it be different on
    // other platforms or CPUs?
    EXPECT_CALL(*mgr, handleUserAndMessagePair(_, _, _, _))
        ...
        .WillRepeatedly([mgr](MockSpamBlockTest::PerChatHandleConstRef e,
                              ...
                              const size_t th,
                              const char* n) { mgr->checks(e, it, th, n); });
    for (const auto& i : msgs) mgr->addMessage(i);
